defmodule MxcWeb.Router do
  use MxcWeb, :router

  pipeline :browser do
    plug :accepts, ["html"]
    plug :fetch_session
    plug :fetch_live_flash
    plug :put_root_layout, html: {MxcWeb.Layouts, :root}
    plug :protect_from_forgery
    plug :put_secure_browser_headers
  end

  pipeline :api do
    plug :accepts, ["json"]
  end

  scope "/", MxcWeb do
    pipe_through :browser

    # Dashboard routes
    live "/", DashboardLive, :index
    live "/nodes", NodesLive, :index
    live "/nodes/:id", NodesLive, :show
    live "/workloads", WorkloadsLive, :index
    live "/workloads/:id", WorkloadsLive, :show
  end

  # API routes for CLI and external integrations
  scope "/api", MxcWeb.API do
    pipe_through :api

    get "/nodes", NodeController, :index
    get "/nodes/:id", NodeController, :show

    get "/workloads", WorkloadController, :index
    post "/workloads", WorkloadController, :create
    get "/workloads/:id", WorkloadController, :show
    post "/workloads/:id/stop", WorkloadController, :stop

    get "/cluster/status", ClusterController, :status
  end

  # Enable LiveDashboard and Swoosh mailbox preview in development
  if Application.compile_env(:mxc, :dev_routes) do
    # If you want to use the LiveDashboard in production, you should put
    # it behind authentication and allow only admins to access it.
    # If your application does not have an admins-only section yet,
    # you can use Plug.BasicAuth to set up some basic authentication
    # as long as you are also using SSL (which you should anyway).
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through :browser

      live_dashboard "/dashboard", metrics: MxcWeb.Telemetry
      forward "/mailbox", Plug.Swoosh.MailboxPreview
    end
  end
end
